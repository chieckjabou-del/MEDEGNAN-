...
export default function OfferCard({ offer }: { offer: Offer }) {
  return (
    <div className="flex flex-col border border-line bg-paper p-6 md:p-8">
      <span className="font-mono text-xs uppercase tracking-wide text-ink-soft">{offer.category}</span>
      ...
      <Link
        href={offer.cta.href}
        className="mt-6 inline-flex w-fit items-center gap-2 text-sm font-medium text-ink underline decoration-accent decoration-2 underline-offset-4 hover:text-accent-hover"
      >
        {offer.cta.label} →
      </Link>
    </div>
